
undefined8 check_trust(long param_1,int param_2)

{
  long lVar1;
  code *pcVar2;
  int iVar3;
  int iVar4;
  int iVar5;
  X509 *a;
  undefined8 uVar6;
  X509_NAME *pXVar7;
  long lVar8;
  X509 *a_00;
  
  lVar1 = *(long *)(param_1 + 0xf0);
  iVar3 = OPENSSL_sk_num(*(undefined8 *)(param_1 + 0x98));
  iVar5 = param_2;
  if (((lVar1 == 0) || ((*(byte *)(lVar1 + 0x28) & 5) == 0)) || (param_2 < 1)) {
    if (iVar3 <= param_2) goto LAB_00590502;
  }
  else {
    if (iVar3 <= param_2) {
LAB_00590502:
      if (iVar3 != param_2) {
        return 3;
      }
      if ((*(byte *)(*(long *)(param_1 + 0x20) + 0x1a) & 8) == 0) {
        return 3;
      }
      a = (X509 *)OPENSSL_sk_value(*(undefined8 *)(param_1 + 0x98),0);
      ERR_set_mark();
      pcVar2 = *(code **)(param_1 + 0x78);
      pXVar7 = X509_get_subject_name(a);
      lVar8 = (*pcVar2)(param_1,pXVar7);
      ERR_pop_to_mark();
      if (lVar8 == 0) {
        return 0xffffffff;
      }
      iVar5 = 0;
      do {
        iVar3 = OPENSSL_sk_num(lVar8);
        if (iVar3 <= iVar5) {
LAB_00590671:
          OPENSSL_sk_pop_free(lVar8,X509_free);
          return 3;
        }
        a_00 = (X509 *)OPENSSL_sk_value(lVar8,iVar5);
        iVar3 = X509_cmp(a_00,a);
        if (iVar3 == 0) {
          if (a_00 != (X509 *)0x0) {
            iVar5 = X509_up_ref(a_00);
            if (iVar5 == 0) {
              OPENSSL_sk_pop_free(lVar8,X509_free);
              return 0xffffffff;
            }
            OPENSSL_sk_pop_free(lVar8,X509_free);
            iVar5 = X509_check_trust(a_00,*(int *)(*(long *)(param_1 + 0x20) + 0x24),0);
            if (iVar5 != 2) {
              OPENSSL_sk_set(*(undefined8 *)(param_1 + 0x98),0,a_00);
              X509_free(a);
              *(undefined4 *)(param_1 + 0x94) = 0;
              goto LAB_005904ae;
            }
            X509_free(a_00);
            iVar5 = 0;
            goto LAB_00590535;
          }
          goto LAB_00590671;
        }
        iVar5 = iVar5 + 1;
      } while( true );
    }
    uVar6 = check_dane_issuer(param_1,param_2);
    if ((int)uVar6 != 3) {
      return uVar6;
    }
  }
  do {
    a = (X509 *)OPENSSL_sk_value(*(undefined8 *)(param_1 + 0x98),iVar5);
    iVar4 = X509_check_trust(a,*(int *)(*(long *)(param_1 + 0x20) + 0x24),0);
    if (iVar4 == 1) goto LAB_005904ae;
    if (iVar4 == 2) {
      if (iVar5 < 0) {
        iVar5 = *(int *)(param_1 + 0xac);
      }
      else {
LAB_00590535:
        *(int *)(param_1 + 0xac) = iVar5;
      }
      if (a == (X509 *)0x0) {
        a = (X509 *)OPENSSL_sk_value(*(undefined8 *)(param_1 + 0x98),iVar5);
      }
      *(X509 **)(param_1 + 0xb8) = a;
      *(undefined4 *)(param_1 + 0xb0) = 0x1c;
      iVar5 = (**(code **)(param_1 + 0x38))(0,param_1);
      if (iVar5 != 0) {
        return 3;
      }
      return 2;
    }
    iVar5 = iVar5 + 1;
  } while (iVar5 < iVar3);
  if ((*(byte *)(*(long *)(param_1 + 0x20) + 0x1a) & 8) == 0) {
    return 3;
  }
LAB_005904ae:
  if ((lVar1 != 0) && (iVar5 = OPENSSL_sk_num(*(undefined8 *)(lVar1 + 8)), 0 < iVar5)) {
    if (*(int *)(lVar1 + 0x30) < 0) {
      *(int *)(lVar1 + 0x30) = param_2;
    }
    if (*(int *)(lVar1 + 0x2c) < 0) {
      return 3;
    }
  }
  return 1;
}

