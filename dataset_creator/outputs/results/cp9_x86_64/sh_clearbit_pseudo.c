...

{
  ...
  if ((param_2 < 0) || (DAT_00932ce8 <= param_2)) {
    ...
    OPENSSL_die("assertion failed: list >= 0 && list < sh.freelist_size","../crypto/mem_sec.c",0x14d
               );
  }
    ...
    if ((bVar1 >> (ulong)uVar3 & 1) != 0) {
      *pbVar2 = ~(byte)(1L << (sbyte)uVar3) & bVar1;
      return;
                    ...
                    /* WARNING: Subroutine does not return */
    OPENSSL_die("assertion failed: TESTBIT(table, bit)","../crypto/mem_sec.c",0x151);
  }
                    /* WARNING: Subroutine does not return */
  OPENSSL_die("assertion failed: bit > 0 && bit < sh.bittable_size","../crypto/mem_sec.c",0x150);
}
