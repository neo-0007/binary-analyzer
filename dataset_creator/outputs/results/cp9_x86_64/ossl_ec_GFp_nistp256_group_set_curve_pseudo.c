...

{
  ...
  if (ret_01 != (BIGNUM *)0x0) {
    BN_bin2bn(nistp256_curve_params,0x20,ret);
    BN_bin2bn(nistp256_curve_params + 0x20,0x20,ret_00);
    BN_bin2bn(nistp256_curve_params + 0x40,0x20,ret_01);
    iVar1 = BN_cmp(ret,param_2);
    if (((iVar1 == 0) && (iVar1 = BN_cmp(ret_00,param_3), iVar1 == 0)) &&
       (iVar1 = BN_cmp(ret_01,param_4), iVar1 == 0)) {
      *(code **)(param_1 + 0x88) = BN_nist_mod_256;
      uVar2 = ossl_ec_GFp_simple_group_set_curve(param_1,param_2,param_3,param_4,param_5);
    }
    else {
      ERR_new();
      ERR_set_debug("../crypto/ec/ecp_nistp256.c",0x78b,"ossl_ec_GFp_nistp256_group_set_curve");
