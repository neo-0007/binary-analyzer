
int _nss_files_getservbyport_r
              (int param_1,char *param_2,long param_3,undefined8 param_4,ulong param_5,int *param_6)

{
  undefined4 uVar1;
  int iVar2;
  undefined4 uVar3;
  FILE *__stream;
  long in_FS_OFFSET;
  undefined8 local_48;
  long local_40;
  
  local_40 = *(long *)(in_FS_OFFSET + 0x28);
  __stream = (FILE *)__nss_files_fopen("/etc/services");
  if (__stream == (FILE *)0x0) {
    iVar2 = (*(int *)(in_FS_OFFSET + -0x58) != 0xb) - 2;
  }
  else {
    uVar1 = *(undefined4 *)(in_FS_OFFSET + -0x58);
    if (param_5 < 2) {
      *param_6 = 0x22;
      iVar2 = -2;
    }
    else {
LAB_007721a0:
      do {
        do {
          iVar2 = __nss_readline(__stream,param_4,param_5,&local_48);
          if (iVar2 == 2) {
            iVar2 = 0;
            *(undefined4 *)(in_FS_OFFSET + -0x58) = uVar1;
            goto LAB_007721fe;
          }
          if (iVar2 != 0) {
LAB_007721ec:
            *param_6 = iVar2;
            iVar2 = (iVar2 != 0x22) - 2;
            goto LAB_007721fe;
          }
          uVar3 = _nss_files_parse_servent(param_4,param_3,param_4,param_5,param_6);
          iVar2 = __nss_parse_line_result(__stream,local_48,uVar3);
          if (iVar2 != 0) {
            if (iVar2 != 0x16) goto LAB_007721ec;
            goto LAB_007721a0;
          }
          *(undefined4 *)(in_FS_OFFSET + -0x58) = uVar1;
        } while (*(int *)(param_3 + 0x10) != param_1);
        if (param_2 == (char *)0x0) break;
        iVar2 = strcmp(*(char **)(param_3 + 0x18),param_2);
      } while (iVar2 != 0);
      iVar2 = 1;
    }
LAB_007721fe:
    fclose(__stream);
  }
  if (local_40 == *(long *)(in_FS_OFFSET + 0x28)) {
    return iVar2;
  }
                    /* WARNING: Subroutine does not return */
  __stack_chk_fail_local();
}

