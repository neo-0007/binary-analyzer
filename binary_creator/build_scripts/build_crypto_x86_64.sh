...
SRC_DIR="${BASE_DIR}/src/crypto"
BUILD_DIR="${BASE_DIR}/outputs/stripped"
...
compile_for_arch() {
    ...
    $compiler -std=c++17 -O2 -static -static-libstdc++ -static-libgcc "$src" -o "$outfile" -lssl -lcrypto -lsodium

    # Check if compilation succeeded
    if [ $? -ne 0 ]; then
        echo "[WARN] Compilation failed for $src. Skipping..."
        return 1
    fi
...
for file in "${SRC_DIR}"/*.cpp; do
    ...
    outpath="${BUILD_DIR}/x86_64/${filename}_x86_64_stripped"
    compile_for_arch "$file" "x86_64" "$outpath"

    #compile but continue on error:
    if ! compile_for_arch "$file" "x86_64" "$outpath"; then
        continue
    fi
done
